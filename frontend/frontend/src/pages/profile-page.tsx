...
const GetUser=async() => {
  ...
  return (
    <Content>
      ...
      <Name>{quoteuser?.first_name} {quoteuser?.last_name}</Name>
      <ScoreGrid>
        <div>Quotes
          <p><OrangeText>{quoteuser?.posts.length}</OrangeText></p>
        </div>
        <div>Quotastic Karma
          <p>32</p>
        </div>
      ...
      <ThreeColumnGrid>
        <div>
          ...
          if(q.user?.id==quoteuser?.id )
            return (
               
              <MostUpvoted key={q?.id} user_id={q.user?.id} id={q.id || 0} img={quoteuser?.avatar} author={quoteuser?.first_name + " " + quoteuser?.last_name} content={q.content || ""} score={q.score || 0} />
            )
          })}
        </div>
        <div>
